...
public partial class _Default : System.Web.UI.Page
{
    ...
    {
        String user_id, email_address_db, password_db, first_name_db;
        if (con.State == ConnectionState.Closed)
            con.Open();
        String cmd = "select email,password,first_name,user_id from [registration] where email='" + email.Text.ToString() + "' and password='" + MyEncrypt(password.Text.ToString()) + "'";
        SqlCommand login = new SqlCommand(cmd, con);
        SqlDataReader read = login.ExecuteReader();
        if (read.Read())
        {
            email_address_db = read.GetValue(0).ToString().Trim();
            password_db = read.GetValue(1).ToString().Trim();
            first_name_db = read.GetValue(2).ToString().Trim();
            user_id = read.GetValue(3).ToString().Trim();
            con.Close();
            if (con.State == ConnectionState.Closed)
                con.Open();
            ...
            if (read2.Read())
            {
                ...
                Session["email"] = email_address_db.ToString();
            ...
            {
                con.Close();
                lblMsgReg.Text = "Your account is inactive. Please activate your account. Please check your email inbox/spam folder for activation link.";
        ...
        {
            con.Close();
            lblMsgReg.Text = "Incorrect Email or Password";
            lblMsgReg.ForeColor = System.Drawing.Color.Red;
        }
    }
