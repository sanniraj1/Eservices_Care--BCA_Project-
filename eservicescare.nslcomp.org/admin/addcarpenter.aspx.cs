...
public partial class admin_addcarpenter : System.Web.UI.Page
{
    ...
    protected void addCarpenter_Click(object sender, EventArgs e)
    {
        
        byte[] myphoto = FileUpload1.FileBytes;
        con.Open();
        SqlCommand checkemail = new SqlCommand("select email from [carpenters] where email =@email", con);
        checkemail.Parameters.AddWithValue("@email", email_address.Text.Trim());
        SqlDataReader read = checkemail.ExecuteReader();
        if (read.HasRows)
        {
            lblMsgReg.Text = "Email Address of carpenter is already exist. Please try with different email address.";
        ...
        else
        {
            con.Close();
            con.Open();
            string insertCmd = "insert into [carpenters](name,per_hour_charge,email,password,mobile,address,photo,created_on) values(@name,@per_hour_charge,@email_address,@password,@mobile,@address,@photo,@created_on)";
